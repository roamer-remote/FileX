...
export default function AdminAgentRunsPage() {
  ...
  }, [message, page, pageSize, filterUserId])
  ...
  useEffect(() => {
    ...
    listAdminUsers({ page: 1, page_size: 100 })
      .then((res) => {
  ...
  const scrollY = useFlexTableBodyScrollY([loading, items.length, page, pageSize], {
    bodyRef,
  })
  ...
  const columns: ColumnsType<AgentRunSummary> = useMemo(
    () => [
      ...
      {
        title: t('agentRuns.colQuestion'),
        dataIndex: 'question_preview',
        ellipsis: true,
      },
      {
        title: t('agentRuns.colStatus'),
        dataIndex: 'status',
        width: 110,
        render: (v: string) => <Tag color={statusColor(v)}>{v}</Tag>,
      },
      {
        title: t('agentRuns.colDuration'),
        dataIndex: 'duration_ms',
        width: 100,
        render: (v?: number | null) => (v != null ? `${(v / 1000).toFixed(1)}s` : '—'),
      },
      {
        title: t('agentRuns.colThread'),
        dataIndex: 'thread_id',
        width: 160,
        ellipsis: true,
        render: (v?: string | null) => v || '—',
      ...
      <div className="admin-panel agent-run-panel">
        <header className="admin-header admin-header--compact agent-run-panel__header">
          <div className="ah-title-group">
            ...
            <span className="ah-sub agent-run-panel__sub">{t('admin.agentRuns.subtitle')}</span>
          </div>
          <div className="ah-toolbar agent-run-panel__toolbar">
            <Space>
              ...
              <Button
                danger
                size="small"
                disabled={!canDeleteAgentRuns(selectedRowKeys, acting)}
            ...
            <Spin spinning={loading} className="fl-spin">
              <div className="fl-table-host">
                <Table<AgentRunSummary>
                  className="agent-run-table fl-file-table"
                  rowKey="id"
                  size="small"
                  tableLayout="fixed"
                  columns={columns}
                  ...
                  rowSelection={{
                    ...
                    getCheckboxProps: () => ({ disabled: acting }),
                  }}
                  locale={{
                    emptyText: <Empty className="agent-run-empty" description={t('agentRuns.empty')} />,
                  }}
                  onRow={(row) => {
                    const isOwn = row.username && row.username === currentUsername
                    return {
                      onClick: (event) => {
                        const target = event.target as HTMLElement
                        if (isAgentRunRowSelectionClick(target)) {
                          return
                        }
                        if (!isOwn) return
          ...
          {!loading && total > 0 && (
            <div className="fl-footer">
              <Pagination
                ...
                showTotal={(t2) => t('common.paginationTotal', { total: t2 })}
              />
            </div>
          )}
        </div>
